...
| 工具 | 入参 | 返回 | 使用时机 |
| --- | --- | --- | --- |
| `search_capabilities` | `query`：1—128 字关键词 | 匹配的普通 Skill 和应用；不包含应用内部 Skill | 已知任务或大致用途时优先使用 |
...
普通 Skill 搜索范围包括名称、简介、分类和最新发布版本的标签；应用搜索使用独立应用目录中的名称、简介、分类和关键词。没有返回的对象不得通过猜测 slug 或应用标识访问。
