...
- 普通 Skill 在详情页维护发现触发词；内置应用在应用页维护触发词。应用 Skill 不单独配置触发词，也不进入企业助手检索。
- 需求探索应用 Skill 必须包含完整 Grill Me 决策树和 frontier 协议；新基线的访谈协议 1.1 优先使用 WorkBuddy 原生问答弹窗，正确区分单选、多选与文本降级，旧协议 1.0 仍支持选择和回退。
